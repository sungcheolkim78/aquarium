...
**Hidden species / population-scale-reduced instances are excluded automatically, with no new code:** three.js's `InstancedMesh.raycast()` only tests instances `0..mesh.count` (the *drawn* count, already lowered by `setPopulationScale`/`setVisible`'s effect on `mesh.count` — see `fish.ts`), and `Raycaster.intersectObjects` skips any object with `visible === false` (what `FishSchool.setVisible(false)` sets). This satisfies the AC "숨겨진 어종이나 비활성 인스턴스를 잘못 선택하지 않는다" for free; a plan task should still add a regression test pinning this down.

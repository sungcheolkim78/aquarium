...
각 body plan은 geometry key에 따라 독립 builder로 디스패치한다. 현재 `lowpoly-fish`, `lowpoly-shark`, `lowpoly-seahorse`, `lowpoly-turtle`을 지원하며, 상어는 비대칭 꼬리엽과 등지느러미를, 해마는 수직 몸통·주둥이·말린 꼬리를, 거북이는 등껍질과 지느러미발을 가진 별도 shape를 사용한다. fish·상어·해마·거북이 builder는 `web/src/creatures/geometry/`에 분리되어 있다. `FishSchool`은 `locomotion: "swim" | "hover"`를 지원하며, hover 생물은 anchor의 X/Z를 유지하고 Y축으로만 완만하게 진동한다.
